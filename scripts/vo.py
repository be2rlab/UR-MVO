...
class vo(Node):
    def __init__(self, images_path=None, gt_path=None, results_path=None):
        ...
        cmd = f"evo_ape tum {gt_path} {poses_path} --save_results {os.path.join(results_path, 'res.zip')} --align --correct_scale --pose_relation trans_part --plot --plot_mode xyz --save_plot {os.path.join(results_path, 'plot.pdf')}"
        sp.call(cmd.split(" "))
