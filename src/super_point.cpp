...
bool SuperPoint::infer(const cv::Mat &image, const cv::Mat &mask,
  ...
  if (!process_output(buffers, features, mask)) {
    return false;
  }
  return true;
}
...
void SuperPoint::filter_points(std::vector<std::vector<int>> &keypoints,
                               std::vector<float> &scores, const cv::Mat &mask,
...
bool SuperPoint::process_output(
    ...
    Eigen::Matrix<double, 259, Eigen::Dynamic> &features, const cv::Mat &mask) {
  keypoints_.clear();
  descriptors_.clear();
  ...
  if (mask.empty())
    ...
    filter_points(keypoints_, scores_vec, mask, semi_feature_map_h,
                  semi_feature_map_w);

  top_k_keypoints(keypoints_, scores_vec, super_point_config_.max_keypoints);
  // std::cout << "super point number is " << std::to_string(scores_vec.size())
  // << std::endl;
...
bool SuperPoint::deserialize_engine() {
  ...
  std::cout << super_point_config_.engine_file.c_str() << std::endl;
  if (file.is_open()) {
